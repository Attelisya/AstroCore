...
public class AstroOverdriveTurbines extends WorkableElectricMultiblockMachine implements ITieredMachine, ITurbineMachine {
    ...
    public AstroOverdriveTurbines(IMachineBlockEntity holder, int tier) {
        super(holder);
        this.tier = tier;
        this.BASE_EU_OUTPUT = GTValues.V[tier] * 8;
    }
    ...
    public static ModifierFunction recipeModifier(@NotNull MetaMachine machine, @NotNull GTRecipe recipe) {
        if (!(machine instanceof AstroOverdriveTurbines turbineMachine)) {
            return RecipeModifier.nullWrongType(AstroOverdriveTurbines.class, machine);
        }
